...
exports.generateUsers = async () => {
  try {
    ...
    if (foundUsers.length === 0) {
      ...
      const user1 = new User({ username: 'Romaine_Hettinger', password: hashedPassword, role: 'User' });
      await user1.save();

      const user2 = new User({ username: faker.internet.userName(), password: hashedPassword, role: 'User' });
      await user2.save();

      const user3 = new User({ username: 'Amelia_Marks', password: hashedPassword, role: 'Admin' });
      await user3.save();
